...
class cylinder_asym(nn.Module):
    ...
    def forward(self, train_pt_fea_ten, train_vox_ten, batch_size):
        ...
        sparse_features, predictions = self.cylinder_3d_spconv_seg(features_3d, coords, batch_size)
        ...
        for k in range(batch_size):
            ...
            sparse_features_conv_tensor_list.append(spconv.SparseConvTensor(sparse_features_padded[k], coords_padded[k].int(),self.sparse_shape, 1))
            
        out = self.sparse_conv_lstm_net(sparse_features_conv_tensor_list, coords, 1)
